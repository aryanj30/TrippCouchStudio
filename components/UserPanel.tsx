  ...
  return (
    <div className="flex h-screen bg-[#F8FAFC] overflow-hidden pt-28">
      ...
      <main className="flex-1 p-6 lg:p-10 overflow-y-auto">
                      ...
                      <div key={order.id} className="bg-white p-6 rounded-2xl border shadow-sm">
                          ...
                          {order.items.map((item, i) => (
                              <div key={i} className="flex gap-4 items-center mb-2">
                                  ...
                                  <div><p className="font-bold text-sm">{item.title}</p><p className="text-xs text-gray-500">{item.price}</p></div>
                              </div>
                          ))}
                      </div>
                  ))}
                  ...
                  {cart.length === 0 ? <p className="text-slate-400">Cart is empty.</p> : (
                      <div className="grid lg:grid-cols-3 gap-8">
                          <div className="lg:col-span-2 space-y-4">
                              {cart.map(item => (
                                  <div key={item.cartId} className="bg-white p-4 rounded-xl border flex gap-4 items-center">
                                      <img src={item.imageUrl} className="w-16 h-16 rounded object-cover"/>
                                      <div className="flex-1"><h3 className="font-bold">{item.title}</h3><p className="text-blue-600 font-bold">{item.price}</p></div>
                                      <button onClick={() => removeFromCart(item.cartId)} className="text-red-400 hover:text-red-600"><Trash2 size={20}/></button>
                                  </div>
                              ))}
                          </div>
                          <div className="bg-white p-6 rounded-2xl border h-fit sticky top-4">
                              <h3 className="font-bold mb-4">Summary</h3>
                              <div className="flex justify-between mb-6 text-sm"><span className="text-slate-500">Items</span><span>{cart.length}</span></div>
                              <button onClick={handleCheckout} className="w-full bg-blue-600 text-white py-3 rounded-xl font-bold hover:bg-blue-700">Request Order</button>
                          </div>
                      </div>
                  )}
              </div>
          ...
          {activeTab === 'settings' && (
              <div className="max-w-xl bg-white p-8 rounded-2xl border">
                  <h2 className="text-2xl font-bold mb-6">Profile</h2>
                  <div className="space-y-4">
                      <div><label className="text-xs font-bold text-slate-500 uppercase">Name</label><input value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} className="w-full border p-2 rounded mt-1"/></div>
                      <div><label className="text-xs font-bold text-slate-500 uppercase">Phone</label><input value={formData.phone} onChange={e => setFormData({...formData, phone: e.target.value})} className="w-full border p-2 rounded mt-1"/></div>
                      <button onClick={handleSaveProfile} disabled={isSaving} className="bg-slate-900 text-white px-6 py-2 rounded-lg font-bold text-sm hover:bg-slate-800">{isSaving ? 'Saving...' : 'Save Changes'}</button>
                  </div>
              </div>
          )}
      </main>
