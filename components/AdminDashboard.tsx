...
// --- Editor Modal for Deep Editing Ad Cards ---
const EditorModal = ({ 
...
const AdminDashboard: React.FC<{ onNavigate: (page: string) => void }> = ({ onNavigate }) => {
  ...
  const handleDeleteConsultation = async (leadId: string) => {
      if (confirm("Delete this lead?")) await db.collection('consultations').doc(leadId).delete();
  };
  ...
  const deleteAdCard = (id: string) => {
      if(window.confirm("Delete?")) setFormData(prev => ({ ...prev, adPlatform: { ...prev.adPlatform, cards: prev.adPlatform.cards.filter(c => c.id !== id) } }));
  };
