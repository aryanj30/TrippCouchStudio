...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const addToCart = async (item: AdCardData) => {
    ...
    // Determine the active price (discounted or regular)
    // Remove symbols to ensure we store a somewhat raw string if needed, 
    // but the app uses formatted strings everywhere. We keep the formatted string for display.
    const priceToUse = item.pricing?.discounted || item.price;

    const newItem: CartItem = {
        ...item,
        price: priceToUse, // Override the price with the detailed price if available
  ...
  const checkout = async () => {
      ...
      // Format back to currency string for display
